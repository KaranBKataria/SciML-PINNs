...
# Define axes object
fig, ax = plt.subplots(
    4, 2, figsize=(6, 10), constrained_layout=True, sharex=True, sharey=True
)
...
for n_sample, index in zip(range(15, 70, 15), range(0, 4)):
    ...
    samples = qmc.scale(
        samples, 0, 20
    )  # Scale from a unit interval [0,1] (default) to [0,20]
    ...
    ax[index][0].legend(fontsize=15)
    ax[index][0].tick_params(axis='both', which='major', labelsize=15)
    ...
    ax[index][1].scatter(
        ...
        marker=".",
        color="red",
        label="$N_{\mathcal{C}}$" + f"={n_sample} (Uniform random)",
    )
    ax[index][1].legend(fontsize=15)
    ax[index][1].tick_params(axis='both', which='major', labelsize=15)
    # Only add x-axis label on the last row of subplots
    if index == 3:
        ax[index][0].set_xlabel("Temporal domain ($\mathcal{T}$)", fontsize=15)
        ax[index][1].set_xlabel("Temporal domain ($\mathcal{T}$)", fontsize=15)
