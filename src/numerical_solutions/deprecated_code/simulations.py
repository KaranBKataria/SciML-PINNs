...
# Define flag to control exporting data
EXPORT: bool = False
...
for iter, (damping, inertia) in tqdm(enumerate(hyperparameter_grid)):
    ...
    file_name: str = (
        "inertia_" + str(round(inertia, 2)) + "_damping_" + str(round(damping, 2))
    )
    ...
    ax[0].scatter(
        times, solution_noisy[0, :], marker=".", color="steelblue", label="Noisy data"
    )
    ...
    ax[1].scatter(
        times, solution_noisy[1, :], marker=".", color="steelblue", label="Noisy data"
    )
    ...
    if EXPORT:
        ...
        if CONTROLLERS:
            plt.savefig(
                PATH / "visualisations" / "numerical_solutions" / "controllers" / file_name,
                format="pdf",
                bbox_inches="tight",
            )
        else:
