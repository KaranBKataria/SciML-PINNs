...
def swing_equation(
        inertia: float, damping: float, phase_angle: float, angular_frequency: float,
        mechanical_power: float, voltage_magnitude: float, include_controllers: bool = False,
        *voltages, *phase_angles, *susceptances, **kwargs) -> np.array:
    """
    ...
    passed into a numerical solver (e.g. RK45).
        ...
        Inputs:
                ...
                phase_angle: The phase angle for the given generator (rads)
                angular_frequency: The angle frequency for the given generator (rads/s)
                ...
                voltages: (*args arguements) The voltages of the other generators in the power system
                phase_angles: (*args arguements) The phase angles of the other generators in the power system
                susceptances: (*args arguement) The susceptance values between other generators and the given generator
    ...
    for v, delta, B in zip(voltages, phase_angles, susceptances):
        total_electrical_output += B * voltage_magnitude * v * sin(phase_angle - delta)
...
def swing_ODEs(time: float, state: np.array) -> float:
    ...
